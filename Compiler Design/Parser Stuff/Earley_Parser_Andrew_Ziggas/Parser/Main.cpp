...
void makeTree(Item item, TreeNode Node, vector<Token>tokenList) {
	if (item.state == "Initial") {
		...
		cout << "Scan" << endl;
		TreeNode temp = TreeNode(item.rhs.at(item.dpos - 1),tokenList.at(item.column-1).getSym());
		Node.children.insert(Node.children.begin(), temp);
		makeTree(*item.Source, Node, tokenList);
	}
		...
		cout << "Complete" << endl;
		cout << "STUFF HERE: " << &item.Source << endl;
		cout << "STUFF HERE Too: " << item.Source->dpos << endl;
		Item complete = *item.Source;
		Item partial = *item.Partial;
		TreeNode Q = TreeNode(complete.lhs, tokenList.at(complete.column - 1).getSym());
		Node.children.insert(Node.children.begin(), Q);
		makeTree(partial, Node, tokenList);
	}
	return;
}
...
int main(int argc, char* args[]) {
	...
	*/
					...
					for (Item item : T[pair<int, int>(i, j)]) {
							...
							for (int k = 0; k <= j; k++) {
								...
								for (Item tempItem : T[pair<int, int>(j, k)]) {
									...
									if (tempItem.dpos != tempItem.rhs.size() && item.lhs == tempItem.rhs.at(tempItem.dpos)) {
										Item newItem = Item(tempItem.lhs, tempItem.rhs, tempItem.dpos + 1, procNum, "Complete", i, k, &item, &tempItem);
										if (T[pair<int, int>(i, k)].find(newItem) == T[pair<int, int>(i, k)].end()) {
											loopCheck = false;
											done = false;
											procNum++;
											if (newItem.lhs == "Start" && newItem.dpos == 1 && i == tokenList.size() && k == 0) {
												is_inFlag = true;
												wasInFlag = true;
												endItem = newItem;
											}
						...
						//cout << "SCAN" << endl;
							...
							for (pair<string, regex> t : terminals) {
								if (t.first.compare(item.rhs.at(item.dpos)) == 0) {
									Item item2 = Item(item.lhs, item.rhs, item.dpos + 1, procNum, "Scan", i+1, j, &item);
						...
						//GIT GUD YOUR LESS THAN COMPARATOR BE JANK
							...
							for (string str : nonTerminals[item.rhs.at(item.dpos)]) {
								if (str == "lambda") {
									Item item2 = Item(item.rhs.at(item.dpos), {}, 0, procNum, "Predict", i, i, &item);
									...
									Item item2 = Item(item.rhs.at(item.dpos), split, 0, procNum, "Predict", i, i, &item);
									//cout << i << endl;
	...
	if (is_inFlag) {
		...
		//TreeNode startNode = TreeNode("blah","blah");
